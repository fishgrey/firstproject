    ...
    if(strlen($query) >= $min_length){ 
        ...
        $query = mysql_real_escape_string($query);
          $raw_results = mysql_query("SELECT * FROM sales
            WHERE (`salesID` LIKE '%".$query."%') OR(`bookID` LIKE '%".$query."%') OR (`bookName` LIKE '%".$query."%')") or die(mysql_error());
        if(mysql_num_rows($raw_results) > 0){ ?>
         <table class="table table-bordered table-hover table-striped">
           ...
           <?php while($results = mysql_fetch_assoc($raw_results)){?>
            <tr>
                                        <td><?php echo"{$results['salesID']}";?></td>
                                        ...
                                        <td><?php echo"{$results['total_sales']}";?></td>
                                        <td><form action="updatesales.php" method="GET">
                                        <input type="hidden" name="salesID" value="<?php echo $results['salesID'];?>">
                                        <input type="submit" name="submit" class="btn btn-sm btn-primary" value="Edit"></form></td>
                                        <td>
                                        <form action="connection_deletesales.php" method="POST">
                                        <input type="hidden" name="salesID" value="<?php echo$results['salesID'];?>">
...
?><p><a href="sales.php" class="btn btn-sm btn-info">Back</a></p>
                <!-- /.row -->
